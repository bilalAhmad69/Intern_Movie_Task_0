...
const getMovieRating = async (req, res) => {
  ...
  try {
    const movieRating = await Rating.find({ movie: req.params.id })
      .select("rating -_id")
      .populate("user", "name email -_id")
      .populate("movie", "name genre -_id");
    if (movieRating.length < 1)
      return res.status(404).send("No one Rated this Movie");
...
const postRating = async (req, res) => {
  ...
  try {
    ...
    const isMovieRated = await Rating.find().and([
      { movie: movieId },
      { user: userId },
    ]);
    if (isMovieRated.length > 0)
      return res.status(400).send("You rated this movie once");
